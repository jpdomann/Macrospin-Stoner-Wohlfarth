...
H0 = 3e4;
[H1_stat,H2_stat,H3_stat] = Create_Field([H0,0,0],...
    {'cos','const','const'},nPtsPerCycle_static,npts_static);
[H1_dyn,H2_dyn,H3_dyn] = Create_Field([H0,0,0],...
    {'cos','const','const'},nPtsPerCycle_dynamic,npts_dynamic);

%Appliedd strain (-)
SMax = 0e-6;
[S1_stat,S2_stat,S3_stat,S4_stat,S5_stat,S6_stat] = Create_Field([SMax,0,0,0,0,0],...
    {'cos','const','const','const','const','const'},nPtsPerCycle_static,npts_static);
[S1_dyn,S2_dyn,S3_dyn,S4_dyn,S5_dyn,S6_dyn] = Create_Field([SMax,0,0,0,0,0],...
    {'sin','const','const','const','const','const'},nPtsPerCycle_dynamic,npts_dynamic);

%SOT current (dynamic fields only)
IMax = 0e4;  
[sigma1,sigma2,sigma3] = Create_Field([0,IMax,0],...
...
grid on
[cg,hyst] = centroid([H1_stat(:),n.State.Stat_m1(:)]);
hold on
plot(cg(1),cg(2),'r*')
